...
- Каждый бот представляет из себя полность законченный микорсервис с единой точкой входа `/event`
...
- Бот оформляется как контейнер с прилагаеммым docker-compose.yml, для запуска которого должно хватить `git clone ... && docker-compose up -d`
...
- Бот не плучит след. сообщения пока не ответит на предыдущее.
...
- функции поиска подкасту (шоунотам и логам)
