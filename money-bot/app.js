...
const currency_RegExp = {
    ...
    btc: /(((BTC)\s?)((\d+\s\d+\s\d+|\d+\s\d+|\d+)((,|\.)\d+)?)(\s?(миллион(а|ов)?|миллиард(а|ов)?|млн|млрд|тысячи|тыс|тысяч|сотен|сотни|k|к)\.?\s?)?|((\d+\s\d+\s\d+|\d+\s\d+|\d+)((,|\.)\d+)?(\s?(миллион(а|ов)?|миллиард(а|ов)?|млн|млрд|тысячи|тыс|тысяч|сотен|сотни|k|к)\.?\s?)?(\s?(BTC|биткоин(а|ов)?|биток|битков))))/i
};
let currency = {
    USD_EUR: 0,
    USD_UAH: 0,
...
app.post('/event', function(req, res) {
    ...
    waterfall([
        ...
        function(v, c, callback){
            ...
            v = v.replace(/бакс/gi, '')
            v = v.replace(/(биткоин(а|ов)?|биток|битков)/gi, '')
